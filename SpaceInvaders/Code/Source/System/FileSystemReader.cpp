...
JSONFileSystemParser::JSONFileSystemParser(const std::stringstream &contents)
{
	...
	ASSERT_DESCRIPTION(!m_Document.Parse<0>(contents.str().c_str()).HasParseError(), "A problem has happened parsing the input file");
}
...
void JSONFileSystemParser::Init()
{
	...
	{
		ASSERT_DESCRIPTION(itr->value.IsObject(), "Not supported JSON Content, we are only providing support to JSON where the root is a list of objects");
